    ...
    <>
      ...
      <div className="mt-5 flex flex-col">
        ...
        <input
          ...
          id="PostHeading"
          className="m-2 w-fit bg-black text-white mx-5 rounded-lg p-2"
        ...
        <input
          ...
          className="m-2 w-fit bg-black text-white mx-5 rounded-lg p-2"
          placeholder="Post Links (Array)"
        />
        <hr />
        <button
          onClick={SubmitPost}
          id="postSubmitButton"
          className="m-4 w-fit bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-xl"
        >
          Post
        </button>
      </div>
      ...
      <div className="flex flex-col items-center justify-center">
        <h1 className="mx-2 my-2 text-3xl border-b-2 border-black w-fit">
          Posts
        </h1>
        <div id="latestPosts">
          {all.map((post: any) => {
...
export async function getServerSideProps(context: any) {
  const a = await fetch("https://tech-vave.vercel.app/api/admin", {
    method: "POST",
  });
  const b = await a.json();
  const c = await fetch("https://tech-vave.vercel.app/api/post", {
    method: "GET",
  });
